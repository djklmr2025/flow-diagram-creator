...
El editor tiene: rectángulos, círculos, líneas animadas, rutas, metros animados, personas, portales, imágenes, texto y lápiz.`;
    ...
    const gatewayKey =
      process.env.ARKAIOS_API_KEY || process.env.VITE_AIDA_AUTH_TOKEN;
    const geminiKey =
      process.env.VITE_GOOGLE_API_KEY || process.env.GEMINI_API_KEY;
    ...
    if (gatewayKey) {
      try {
        ...
        console.error("Arkaios error:", e.message);
      }
    }
    ...
    if (!reply) {
      reply =
        "No pude conectarme al servicio IA. Verifica las variables de entorno ARKAIOS_API_KEY o VITE_GOOGLE_API_KEY en Vercel.";
    }
