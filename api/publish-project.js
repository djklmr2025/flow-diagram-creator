...
import { normalizeProjectInPlace, walkElements } from "./_shared/normalize.js";
...
async function uploadProjectImagesInPlace(elements, assetPrefix) {
  ...
  if (imageElems.length > MAX_IMAGES) {
    return {
      ...
      error: "TooManyImages",
      details: [
        `Demasiadas imagenes: ${imageElems.length} (max ${MAX_IMAGES})`,
      ],
    };
  }
  ...
  for (let i = 0; i < imageElems.length; i++) {
    ...
    if (!parsed) {
      return {
        ...
        details: [
          "Se detecto una imagen embebida, pero no es data URL base64 soportada.",
        ],
      };
    }
    ...
    if (bytes > MAX_IMAGE_BYTES) {
      return {
        ...
        error: "ImageTooLarge",
        details: [
          `Imagen demasiado grande (${bytes} bytes). Max por imagen: ${MAX_IMAGE_BYTES}.`,
        ],
      };
    }
    ...
    if (totalBytes > MAX_TOTAL_IMAGE_BYTES) {
      return {
        ...
        details: [
          `Suma de imagenes demasiado grande (${totalBytes} bytes). Max total: ${MAX_TOTAL_IMAGE_BYTES}.`,
        ],
      };
    }
...
export default async function handler(req, res) {
  ...
  if (requiredKey) {
    ...
    const key =
      req.headers["x-publish-key"] || url.searchParams.get("key") || "";
    if (key !== requiredKey) {
  ...
  if (!body || typeof body !== "object") {
    sendJson(res, 400, {
      ...
      details: ["body debe ser un objeto JSON"],
    });
    return;
  }
  ...
  if (!Array.isArray(body.elements)) {
    sendJson(res, 400, {
      ok: false,
      error: "ValidationError",
      details: ["elements debe ser un array"],
    });
    return;
  }
  ...
  if (totalElements > MAX_ELEMENTS) {
    sendJson(res, 400, {
      ...
      error: "ValidationError",
      details: [
        `elements excede el maximo (${MAX_ELEMENTS}): ${totalElements}`,
      ],
    });
    return;
  }
  ...
  const uploadRes = await uploadProjectImagesInPlace(
    body.elements,
    assetPrefix,
  );
  if (!uploadRes.ok) {
